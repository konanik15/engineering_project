...
async function setup() {
    ...
    app.ws("/lobby/:idOrCode", keycloak.protectWS(), async (ws, req) => {
        ...
        try {
            lobby = await Lobby.findById(idOrCode);
        } catch (e) {
        }
        if (lobby)
            foundBy = "id";
        ...
        if (foundBy === "id" && lobby.passwordProtected) {
            const password = req.headers.password ? req.headers.password : req.query.password;
            if (!password) {
            ...
            if (pulledPlayer.leader) {
                ...
                if (newLeader) {
                    ...
                    await Lobby.findOneAndUpdate({
                        _id: lobbyId,
                        'players.wsId': newLeader.wsId
                    }, {'$set': {'players.$.leader': true}}, {new: false});
                    lobby.hasLeader = true;
                    await lobby.save();
                }
            }
        ...
        ws.on("message", async (message) => {
            ...
            switch (type) {
                ...
                case "ready":
                    try {
                        await Lobby.findOneAndUpdate({
                            _id: lobbyId,
                            'players.wsId': ws.id
                        }, {'$set': {'players.$.ready': true}}, {new: false});
                        ws.send(JSON.stringify({type: "readyResult", data: {success: true}}));
                        tempClients = lobbyClients.get(lobbyId);
                        const pulledPlayer = lobby.players.find((player) => player.wsId === ws.id);
                        broadcastToClients(tempClients, JSON.stringify({
                            type: "playerReady",
                            data: {username: pulledPlayer.name}
                        }));
                    } catch (err) {
                        console.error(err);
                ...
                case "unready":
                    try {
                        await Lobby.findOneAndUpdate({
                            _id: lobbyId,
                            'players.wsId': ws.id
                        }, {'$set': {'players.$.ready': false}}, {new: false});
                ...
                case "startGame":
                    ...
                    broadcastToClients(mainMenuClients, JSON.stringify({
    ...
    app.patch("/lobby/reset/:id", async (req, res) => {
        try {
            ...
            broadcastToClients(mainMenuClients, JSON.stringify({type: "lobbyPending", data: {lobbyId: lobby._id}}));
            lobby.players.forEach(p => broadcastToClients(clients, JSON.stringify({
                type: "playerUnready",
