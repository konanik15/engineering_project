...
This websocket connects the client to the lobby he joins. First it checks if the client can join the lobby (isFull?), then it checks if the lobby has a leader. If not then the new client is the leader. All of this is then saved to the db. 
